...
static void buildTable(void) {
  ...
  for (i = 0; i < 64; i++) {
    ...
    j = FILE(i) - RANK(i);
    if (j > 0) {
      controlMask[2][i] = 0x8040201008040201ull >> (j << 3);
    } else {
      controlMask[2][i] = 0x8040201008040201ull << (-j << 3);
    }
    j = FILE(i) - (R8 - RANK(i));
    if (j > 0) {
      controlMask[3][i] = 0x0102040810204080ull << (j << 3);
    } else {
      controlMask[3][i] = 0x0102040810204080ull >> (-j << 3);
    }
  }
  ...
  for (i = 0; i < 64; i++) {
    ...
    for (j = FILE(i) - 1; j <= FILE(i) + 1; j++) {
      if ((FILE(i) == FA && j == -1) || (FILE(i) == FH && j == 8)) {
        continue;
      }
  ...
  for (i = 0; i < 8; i++) {
    adjacentMask[i] = 0;
    if (i > 0) {
      adjacentMask[i] |= 0x0101010101010101ull << (i - 1);
    }
    if (i < 7) {
      adjacentMask[i] |= 0x0101010101010101ull << (i + 1);
    }
...
static void uciControl(void) {
  ...
  for (;;) {
    ...
    if (strcmp(token, "uci") == 0) {
      ...
      processOption(pointer);
    } else if (strcmp(token, "position") == 0) {
      processPOS(position, pointer);
    } else if (strcmp(token, "go") == 0) {
      processGO(position, pointer);
...
void processOption(char *pointer) {
...
void processPOS(POST *position, char *pointer) {
...
void processGO(POST *position, char *pointer) {
...
void setPOS(POST *position, char *epd) {
  D64 KEYLCT;
  int i, j, PCC;
  ...
  if (position->ENPSQR != NOSQ) {
    KEYLCT ^= zobristEnPassant[FILE(position->ENPSQR)];
  }
  if (position->SIDE == BC) {
    KEYLCT ^= ~(0ull);
  }
...
int convertMove(POST *position, char *moveStr) {
  int from, to, type;

  from = ((moveStr[1] - '1') << 3) | (moveStr[0] - 'a');
  to = ((moveStr[3] - '1') << 3) | (moveStr[2] - 'a');
  type = NORMAL;
  if (TPONSQR(position, from) == K && abs(to - from) == 2) {
    type = CASTLE;
  } else if (TPONSQR(position, from) == P) {
    if (to == position->ENPSQR) {
      type = EPCP;
...
void doMove(POST *position, int move, UNDO *undo) {
...
void undoMove(POST *position, int move, UNDO *undo) {
  int SIDE, fsq, tsq, ftp, TTP;
  ...
  switch (move >> 12) {
    ...
    case PMB:
    case PMR:
    case PMQ:position->PCC[fsq] = PC(SIDE, P);
...
int search(POST *position, int ply, int alpha, int beta, int depth, int *pv) {
  ...
  if (depth <= 0) {
    return qSearch(position, ply, alpha, beta, pv);
  }
  nodes++;
  check();
  if (killSearch) {
  ...
  if (checkRepeat(position) && ply) {
    return 0;
  }
  move = 0;
  if (TTR(position->KEYLCT, &move, &score, alpha, beta, depth, ply)) {
    return score;
  }
  if (ply >= MPLY - 1) {
  ...
  if (*pv) {
    ...
    TTS(position->KEYLCT, 0, best, UPPER, depth, ply);
  }
  return best;
}
...
void TTS(D64 KEYLCT, int move, int score, int FLAGS, int depth, int ply) {
  ...
  replace->KEYLCT = KEYLCT;
  replace->DATE = transTableDate;
  replace->move = move;
  replace->score = score;
  replace->FLAGS = FLAGS;
  replace->depth = depth;
...
int generateMob(POST *position, int SIDE) {
  D64 pieces;
  int mob = 0;
...
int evalPawnStruct(POST *position, int SIDE) {
  D64 pieces;
  int from, score = 0;

  for (pieces = PCB(position, SIDE, P); pieces; pieces &= pieces - 1) {
    from = BSF(pieces);
    if (!(passedMask[SIDE][from] & PCB(position, SIDE ^ 1, P))) {
      score += passedBonus[SIDE][RANK(from)];
    }
    if (!(adjacentMask[FILE(from)] & PCB(position, SIDE, P))) {
      score -= 16;
    }
  }
  return score;
...
int evalKingSafety(POST *position, int SIDE) {
  if (!PCB(position, SIDE ^ 1, Q) || position->MATERIAL[SIDE ^ 1] <= 1650) {
    return 0;
  }
  return (-2) * PST[K][KINGSQR(position, SIDE)];
}

int evalPosition(POST *position) {
  int score;

  score = position->MATERIAL[WC] - position->MATERIAL[BC];
  if (score < 300 && score > -300) {
    score += generateMob(position, WC) - generateMob(position, BC);
  }
...
int *genCaptures(POST *position, int *list) {
  D64 pieces, moves;
  int SIDE, from, to;
...
static int ISLEGAL(POST *position, int move) {
  ...
  ftp = TPONSQR(position, fsq);
  TTP = TPONSQR(position, tsq);
  if (ftp == NOTP || (position->PCC[fsq] & 1) != SIDE) {
    return 0;
  }
  if (TTP != NOTP && (position->PCC[tsq] & 1) == SIDE) {
    return 0;
  }
  switch (move >> 12) {
    case NORMAL: break;
    case CASTLE:
      if (SIDE == WC) {
        ...
        if (tsq > fsq) {
          if ((position->CAPFLAGS & 1) && !((position->CLB[WC] | position->CLB[BC]) & 0x0000000000000060ull)) {
            if (!attacked(position, E1, BC) && !attacked(position, F1, BC)) {
              return 1;
            }
          }
        } else {
          if ((position->CAPFLAGS & 2) && !((position->CLB[WC] | position->CLB[BC]) & 0x000000000000000Eull)) {
            if (!attacked(position, E1, BC) && !attacked(position, D1, BC)) {
              return 1;
            }
          }
        }
        ...
        if (tsq > fsq) {
          if ((position->CAPFLAGS & 4) && !((position->CLB[WC] | position->CLB[BC]) & 0x6000000000000000ull)) {
            if (!attacked(position, E8, WC) && !attacked(position, F8, WC)) {
              return 1;
            }
          }
        } else {
          if ((position->CAPFLAGS & 8) && !((position->CLB[WC] | position->CLB[BC]) & 0x0E00000000000000ull)) {
            if (!attacked(position, E8, WC) && !attacked(position, D8, WC)) {
              return 1;
            }
          }
        }
      ...
      return 0;
    case EPCP:
      if (ftp == P && tsq == position->ENPSQR) {
        return 1;
      }
      return 0;
    case EPST:
      if (ftp == P && TTP == NOTP && position->PCC[tsq ^ 8] == NOPC) {
        if ((tsq > fsq && SIDE == WC) || (tsq < fsq && SIDE == BC)) {
          return 1;
        }
      }
      return 0;
  ...
  switch (TPONSQR(position, fsq)) {
    case P: result = pawnAttacks[position->PCC[fsq] & 1][fsq];
      break;
    case N: result = knightAttacks[fsq];
      break;
    case B: result = BISHOPATT(position->CLB[WC] | position->CLB[BC], fsq);
      break;
    case R: result = ROOKATT(position->CLB[WC] | position->CLB[BC], fsq);
      break;
    case Q: result = QUEENATT(position->CLB[WC] | position->CLB[BC], fsq);
      break;
    case K: result = kingAttacks[fsq];
      break;
  }
...
int *genQuietPOS(POST *position, int *list) {
  ...
  if (SIDE == WC) {
    ...
    moves =
      ((((PCB(position, WC, P) & 0x000000000000FF00ull) << 8) & (~(position->CLB[WC] | position->CLB[BC]))) << 8) &
    ...
    moves =
      ((((PCB(position, BC, P) & 0x00FF000000000000ull) >> 8) & (~(position->CLB[WC] | position->CLB[BC]))) >> 8) &
        (~(position->CLB[WC] | position->CLB[BC]));
    while (moves) {
      to = BSF(moves);
      *list++ = (EPST << 12) | (to << 6) | (to + 16);
...
int nextMove(MOVES *moves) {
  int move;

  switch (moves->PHASE) {
    case 0:move = moves->TTMOVE;
      if (move && ISLEGAL(moves->position, move)) {
        moves->PHASE = 1;
        return move;
      }
    case 1:moves->LAST = genCaptures(moves->position, moves->move);
      scoreCaptures(moves);
      moves->NXT = moves->move;
    ...
    case 3:move = moves->KM1;
      if (move && move != moves->TTMOVE && moves->position->PCC[(move >> 6) & 63] == NOPC &&
        ISLEGAL(moves->position, move)) {
        moves->PHASE = 4;
        return move;
      }
    case 4:move = moves->KM2;
      if (move && move != moves->TTMOVE && moves->position->PCC[(move >> 6) & 63] == NOPC &&
        ISLEGAL(moves->position, move)) {
        moves->PHASE = 5;
        return move;
      }
    case 5:moves->LAST = genQuietPOS(moves->position, moves->move);
      scoreQuietPOS(moves);
      moves->NXT = moves->move;
      moves->PHASE = 6;
    case 6:
      while (moves->NXT < moves->LAST) {
        move = sortBest(moves);
        if (move == moves->TTMOVE || move == moves->KM1 || move == moves->KM2) {
          continue;
        }
        return move;
      }
...
void printPV(int score, int *pv) {
  long time;
  char *type, pvStr[256];

  type = "mate";
  if (score < -MEVAL) {
    score = (-MATESCORE - score) >> 1;
  } else if (score > MEVAL) {
    score = (MATESCORE - score + 1) >> 1;
  } else {
    type = "cp";
  }
  convertPV(pv, pvStr);
...
void scoreCaptures(MOVES *moves) {
  int *movep, *valuep;
...
void scoreQuietPOS(MOVES *moves) {
  int *movep, *valuep;
...
int poorCapture(POST *position, int move) {
  D64 attackers, occ, typeX;
  int fsq, tsq, SIDE, ply, type, score[32];

  fsq = move & 63;
  tsq = (move >> 6) & 63;
  if (pieceValue[TPONSQR(position, tsq)] >= pieceValue[TPONSQR(position, fsq)]) {
    return 0;
  }
  if ((move >> 12) == EPCP) {
    return 0;
  ...
  for (ply = 1; attackers & position->CLB[SIDE]; ply++) {
    ...
    for (type = P; type <= K; type++) {
      if ((typeX = PCB(position, SIDE, type) & attackers)) {
        break;
      }
    }
    occ ^= typeX & -typeX;
...
int mvvlva(POST *position, int move) {
...
void TT(POST *position, int move, int depth, int ply) {
  if (position->PCC[(move >> 6) & 63] != NOPC || (move & 0x4000) || (move >> 12) == EPCP) {
    return;
  }
...
void convertStr(int move, char *moveStr) {
...
void convertPV(int *pv, char *pvStr) {
  int *movep;
  char moveStr[6];
...
void constructPV(int *dst, int *src, int move) {
  *dst++ = move;
  while ((*dst++ = *src++)) {
  }
}

int attacked(POST *position, int sq, int SIDE) {
